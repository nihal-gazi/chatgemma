...
export default function ToolCallPill({ toolCall }) {
  ...
  if (name === "web_search" || name === "google_search") {
    ...
    const rCount = args.relationships?.length || 0;
    headerSummary = `User: -${eCount} ent, -${rCount} rel`;
  } else if (name === "sample_code_query") {
    headerSummary = `"${args.query || ""}"${args.library ? ` [${args.library}]` : ""}`;
  } else if (name === "get_code_sample") {
    headerSummary = `\`${args.snippet_id || ""}\``;
...
function formatToolName(name) {
  ...
  if (name === "sample_code_query") return "Sample Code Query";
  if (name === "get_code_sample") return "Get Code Sample";
  return name
    .split("_")
...
function renderToolResponse(name, response) {
  ...
  // Specialized Renderer for Synapser Sample Code Query (Stage 1)
  if (name === "sample_code_query") {
    return <SampleCodeQueryRenderer response={response} />;
  }

  // Specialized Renderer for Synapser Get Code Sample (Stage 2)
  if (name === "get_code_sample") {
    return <GetCodeSampleRenderer response={response} />;
  }
...
function SampleCodeQueryRenderer({ response }) {
  if (!response) return null;
...
function GetCodeSampleRenderer({ response }) {
  ...
  return (
    <div className="kg-results-container synapser-gui-container">
      ...
      {/* 3. Code Viewer with Copy Button */}
      ...
      {/* 4. Collapsible Architectural Breakdown */}
      {long_desc && (
        <div className="brainstorm-original-toggle-area">
          <button
            type="button"
